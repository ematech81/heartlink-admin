...
const LIMIT = 15;
const STATUS_COLORS = { pending: 'bg-yellow-100 text-yellow-700', reviewed: 'bg-blue-100 text-blue-700', actioned: 'bg-red-100 text-red-700', ignored: 'bg-slate-100 text-slate-500' };
const REASON_LABELS = { inappropriate_content: 'Inappropriate Content', harassment: 'Harassment', fake_profile: 'Fake Profile', spam: 'Spam', other: 'Other' };
...
export default function ReportsPage() {
  ...
  const load = useCallback(async (pg = 1) => {
    ...
    try {
      ...
      setReports(data.reports); setTotal(data.total); setTotalPages(data.totalPages); setPage(pg);
  ...
  const handle = async (reportId, status, actionTaken) => {
    ...
    try { await AdminAPI.handleReport(reportId, { status, actionTaken }); setSelected(null); load(page); }
                  ...
                  <tbody>
                    {loading ? (
                      <tr><td colSpan={6} className="text-center py-12 text-slate-400">
                        <span className="w-6 h-6 border-2 border-primary/30 border-t-primary rounded-full animate-spin inline-block" /></td></tr>
                    ) : reports.length === 0 ? (
                      <tr><td colSpan={6} className="text-center py-12 text-slate-400">No {filter} reports</td></tr>
                    ) : reports.map(r => (
                      <tr key={r._id} className={clsx('table-row cursor-pointer', selected?._id === r._id && 'bg-primary/5')} onClick={() => setSelected(r)}>
                        <td className="px-4 py-3"><UserChip user={r.reporter} /></td>
                        <td className="px-4 py-3"><UserChip user={r.reported} banned={r.reported?.isBanned} /></td>
                        <td className="px-4 py-3 text-slate-600">{REASON_LABELS[r.reason] || r.reason}</td>
                        <td className="px-4 py-3"><span className={clsx('badge capitalize', STATUS_COLORS[r.status])}>{r.status}</span></td>
              ...
              <div className="card p-5 space-y-4">
                ...
                <div className="border-t border-slate-100 pt-4">
                  ...
                  <div className="space-y-2">
                    <ActionBtn onClick={() => handle(selected._id, 'ignored',  'none')}     disabled={!!actionId} icon={XCircle}       label="Ignore Report"         cls="text-slate-600 hover:bg-slate-50" />
                    <ActionBtn onClick={() => handle(selected._id, 'actioned', 'warned')}   disabled={!!actionId} icon={AlertTriangle} label="Warn User"             cls="text-yellow-700 hover:bg-yellow-50" />
                    <ActionBtn onClick={() => handle(selected._id, 'actioned', 'suspended')}disabled={!!actionId} icon={Ban}           label="Suspend Account"       cls="text-orange-700 hover:bg-orange-50" />
                    <ActionBtn onClick={() => handle(selected._id, 'actioned', 'banned')}   disabled={!!actionId} icon={Ban}           label="Ban User Permanently"  cls="text-red-700 hover:bg-red-50" />
                    <ActionBtn onClick={() => handle(selected._id, 'reviewed', 'none')}     disabled={!!actionId} icon={CheckCircle}   label="Mark Reviewed"         cls="text-green-700 hover:bg-green-50" />
                  </div>
                </div>
              </div>
            </div>
    ...
    <div className="flex items-center gap-2">
      <div className="w-7 h-7 rounded-full bg-slate-100 flex-shrink-0 overflow-hidden">
        {user.profilePicture
          ...
          : <span className="w-full h-full flex items-center justify-center text-slate-400 text-xs font-bold">{user.name?.[0]?.toUpperCase()}</span>}
      </div>
      <div>
...
function Field({ label, value }) {
  return <div><p className="text-xs text-slate-400 mb-0.5">{label}</p><p className="text-slate-700 capitalize">{value}</p></div>;
}
function ActionBtn({ onClick, disabled, icon: Icon, label, cls }) {
  return (
    <button onClick={onClick} disabled={disabled} className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors ${cls}`}>
