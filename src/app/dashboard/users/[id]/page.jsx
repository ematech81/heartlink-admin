...
import { ArrowLeft, Ban, Trash2, Crown, MapPin, Calendar, Mail, Phone, Heart, MessageSquare, Film, Flag, Shield } from 'lucide-react';
...
export default function UserDetailPage() {
  ...
  const load = async () => {
    try {
      ...
      setData(d);
    } catch (err) { alert(err.message); router.back(); }
  ...
  const update = async (fields) => {
    setSaving(true);
    try { await AdminAPI.updateUser(id, fields); await load(); }
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try { await AdminAPI.deleteUser(id); router.push('/dashboard/users'); }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="card p-6 flex flex-col items-center text-center gap-4">
            ...
            <div className="w-full space-y-2 pt-2 border-t border-slate-100">
              <button onClick={() => update({ isBanned: !user?.isBanned, isActive: user?.isBanned })} disabled={saving}
                className={clsx('w-full flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-colors',
                  user?.isBanned ? 'bg-green-50 text-green-700 hover:bg-green-100' : 'bg-orange-50 text-orange-700 hover:bg-orange-100')}>
                <Ban className="w-4 h-4" />{user?.isBanned ? 'Unban User' : 'Ban User'}
              </button>
              {!user?.isSubscribed && (
                <button onClick={() => update({ isSubscribed: true, subscriptionPlan: 'monthly', subscriptionExpiry: new Date(Date.now() + 30*24*60*60*1000).toISOString() })}
                  disabled={saving} className="w-full flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium bg-purple-50 text-purple-700 hover:bg-purple-100 transition-colors">
                  <Crown className="w-4 h-4" /> Grant 30-Day Premium
                </button>
              )}
              {user?.role !== 'admin' && (
                <button onClick={() => update({ role: 'admin' })} disabled={saving}
...
function Info({ icon: Icon, label, value }) {
  return (
    <div className="flex items-start gap-2">
      ...
      <div><p className="text-xs text-slate-400">{label}</p><p className="font-medium text-slate-700 break-all">{value}</p></div>
    </div>
  );
}
