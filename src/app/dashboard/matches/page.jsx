...
export default function MatchesPage() {
  ...
  useEffect(() => {
    AdminAPI.getMatches().then(setData).catch(console.error).finally(() => setLoading(false));
  }, []);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="card p-6">
                ...
                <div className="space-y-4">
                  ...
                  <FunnelBar label="Recent Chats"   value={data?.recentMessages} max={data?.totalMatches}  color="bg-blue-500" />
                </div>
              </div>
              <div className="card p-6">
                <h3 className="text-sm font-semibold text-slate-700 mb-4">Key Ratios</h3>
                <div className="space-y-3 text-sm">
                  <Ratio label="Like → Match Rate"      value={data?.totalLikes > 0 ? `${((data.totalMatches / data.totalLikes) * 100).toFixed(1)}%` : '—'} />
                  <Ratio label="New Matches (7d)"        value={fmt(data?.recentMatches)} />
                  <Ratio label="Messages Today"          value={fmt(data?.recentMessages)} />
                </div>
                <div className="mt-6 p-4 bg-amber-50 rounded-xl">
                  <p className="text-xs font-semibold text-amber-800 mb-1">⚠ Privacy Notice</p>
                  <p className="text-xs text-amber-700">Individual conversation content is not visible to admins. Only aggregate statistics are shown.</p>
                </div>
              </div>
            </div>
          </>
  ...
  return (
    <div>
      <div className="flex justify-between text-xs text-slate-500 mb-1">
        <span>{label}</span><span className="font-semibold text-slate-700">{(value ?? 0).toLocaleString()}</span>
      </div>
