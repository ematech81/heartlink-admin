...
export default function AnalyticsPage() {
  ...
  const genderData = data?.genderDist?.map(g => ({
    name: g._id?.charAt(0).toUpperCase() + g._id?.slice(1),
    value: g.count,
  })) || [];

  const planData = data?.subscriptionDist?.map(s => ({
    name: s._id?.charAt(0).toUpperCase() + s._id?.slice(1),
    value: s.count,
  })) || [];

  const locationData = data?.locationDist || [];
    ...
    <>
      ...
      <main className="flex-1 p-6 space-y-6 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <MetricCard label="Top City"            value={locationData[0]?.city || '—'}       icon={MapPin}    color="blue" />
        </div>
          ...
          {[7, 14, 30, 60].map(d => (
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                period === d ? 'bg-primary text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
            >
              {d}d
            </button>
        ...
        {locationData.length > 0 && (
          <div className="card p-6">
            <h2 className="text-sm font-semibold text-slate-700 mb-4">Top 10 Cities by Users</h2>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={locationData} layout="vertical" margin={{ top: 0, right: 20, bottom: 0, left: 60 }}>
...
function EmptyState() {
  return <div className="h-48 flex items-center justify-center text-slate-300 text-sm">No data yet</div>;
}
