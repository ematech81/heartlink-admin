...
export default function CommunityPage() {
  ...
  const featurePost = async (post) => {
    setActionId(post._id);
    try {
      await AdminAPI.featurePost(post._id);
      load(page);
    } catch (err) { alert(err.message); }
    finally { setActionId(null); }
  };
    ...
    <div className="card overflow-hidden group">
      ...
      <div className="relative aspect-[4/5] bg-slate-100">
        {post.mediaUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-slate-300 text-xs p-2 text-center">
        ...
        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity
          ...
          <button
            ...
            disabled={busy}
            className="p-1.5 rounded-lg bg-yellow-400 text-yellow-900 hover:bg-yellow-500 transition-colors"
      ...
      <div className="p-2">
        ...
        {post.caption && (
          <p className="text-xs text-slate-400 mt-1 truncate">{post.caption}</p>
        )}
      </div>
    </div>
  );
