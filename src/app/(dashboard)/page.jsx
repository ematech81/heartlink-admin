              ...
              <ResponsiveContainer width="100%" height={210}>
                <LineChart data={growth} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                  ...
                  <Line
                    ...
                    stroke="#FF4B7A" strokeWidth={2} dot={false} activeDot={{ r: 4 }}
                  />
                </LineChart>
              </ResponsiveContainer>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="card p-5">
            ...
            <div className="space-y-2.5">
              <Stat label="Verified Users"    value={`${stats ? Math.round((stats.subscribedUsers / Math.max(stats.totalUsers, 1)) * 100) : 0}% subscribed`} />
              <Stat label="Daily Active Rate" value={`${stats ? Math.round((stats.activeToday / Math.max(stats.totalUsers, 1)) * 100) : 0}% active`} />
              <Stat label="Boost Penetration" value={`${stats ? Math.round((stats.boostedUsers / Math.max(stats.totalUsers, 1)) * 100) : 0}% boosted`} />
            </div>
          </div>
          <div className="card p-5">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Revenue Est. (Naira)</p>
            <div className="space-y-2.5">
