...
export default function SubscriptionsPage() {
  ...
  const cancelSubscription = async (user) => {
    ...
    try {
      await AdminAPI.updateSubscription(user._id, {
  ...
  const extendSubscription = async (user) => {
    const expiry = new Date(
      Math.max(Date.now(), new Date(user.subscriptionExpiry || Date.now())) + 30 * 24 * 60 * 60 * 1000
    );
    setActionId(user._id);
    try {
      await AdminAPI.updateSubscription(user._id, {
    ...
    <>
      ...
      <main className="flex-1 p-6 space-y-6 overflow-y-auto">
        ...
        <div className="card p-4 flex gap-3 items-center">
          <label className="text-sm font-medium text-slate-600">Filter by plan:</label>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-slate-600">Boost</th>
                ...
                {loading ? (
                  ...
                  return (
                    <tr key={u._id} className="table-row">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-slate-100 overflow-hidden flex-shrink-0">
                            {u.profilePicture
                              ...
                              : <span className="w-full h-full flex items-center justify-center text-slate-400 font-bold text-xs">
                      ...
                      <td className="px-4 py-3">
                        {u.isSubscribed ? (
                          <span className={clsx('badge', u.subscriptionPlan === 'yearly'
                          ...
                          <span className="badge bg-orange-100 text-orange-700">⚡ Boosted</span>
                        ) : '—'}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        {u.isBoosted && u.boostExpiry
                          ? <span className="text-xs text-slate-500">Until {new Date(u.boostExpiry).toLocaleDateString()}</span>
                          : '—'}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => extendSubscription(u)}
